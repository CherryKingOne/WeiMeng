...
export default function Text2ImagePage() {
  ...
  const [selectedLightingIndex, setSelectedLightingIndex] = useState(0);

  const text = {
    home: isEn ? 'Home' : '首页',
    workspace: isEn ? 'Workspace' : '工作台',
    title: isEn ? 'Text to Image' : '文生图',
    switchToVideo: isEn ? 'Switch to AI Video' : '切换至AI视频',
    apiPlatform: isEn ? 'API Platform' : 'API开放平台',
    firstDiscount: isEn ? 'First Purchase Offer' : '首购优惠',
    points: isEn ? 'Points' : '积分',
    mode: isEn ? 'Text to Image' : '文生图',
    promptPlaceholder: isEn
      ? 'Describe what you want to generate'
      : '描述你想要生成的内容',
    negativePrompt: isEn ? 'Negative Prompt' : '反向提示词',
    addNegative: isEn ? 'Add negative prompt' : '添加反向提示词',
    negativePlaceholder: isEn
    ...
    styleOptions: isEn
      ...
      : ['写实', '动漫', '插画', '3D渲染', '油画'],
    lightingOptions: isEn
      ? ['Cinematic', 'Natural', 'Soft', 'Rembrandt']
      : ['电影光', '自然光', '柔光', '伦勃朗光'],
    quickNegativeTags: isEn
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 border border-gray-200 rounded-xl transition-all ${
                  showNegativePrompt ? 'bg-gray-50' : 'bg-white'
                }`}
              >
                <span className="text-sm font-medium text-gray-700">{text.negativePrompt}</span>
                <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={showNegativePrompt ? 'M20 12H4' : 'M12 4v16m8-8H4'} />
                </svg>
              </button>
              {showNegativePrompt && (
                <div className="p-4 pt-0">
                  <textarea
                    ...
                    className="w-full h-20 bg-gray-50 rounded-xl border-none resize-none text-sm text-gray-700 placeholder-gray-400 focus:outline-none p-3"
                  />
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="px-3 py-1.5 rounded-full border border-gray-200 hover:border-gray-300 hover:bg-gray-50 text-xs text-gray-600 transition-colors flex items-center gap-1.5"
              >
              ...
              {text.quickNegativeTags.map((tag) => (
                <button
                  key={tag}
                  onClick={() => appendNegativeTag(tag)}
                  className="px-3 py-1.5 rounded-full border border-gray-200 hover:border-gray-300 hover:bg-gray-50 text-xs text-gray-600 transition-colors"
                >
                  {tag}
                </button>
              ))}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">{text.lighting}</span>
                <select
                  value={selectedLightingIndex}
                  onChange={(event) => setSelectedLightingIndex(Number(event.target.value))}
                  className="h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-200"
                >
                  {text.lightingOptions.map((option, index) => (
                    <option key={option} value={index}>
                      {option}
                    </option>
                  ))}
