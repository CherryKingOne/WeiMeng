...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const tabs = [
    ...
    { id: 'model-provider', label: '模型供应商', icon: 'M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16zM3.27 6.96L12 12l8.73-5.04M12 22V12' },
  ...
  const saveCompatibleEditor = () => {
    ...
    if (!modelName || !apiKey || !baseUrl) {
      setCompatError('模型名称、Key、URL 为必填项');
      return;
    }
          ...
          [editingCompatible.modelType]: provider.models[editingCompatible.modelType].map((model) =>
            model.id === editingCompatible.modelId
              ? {
                  ...
                  name: modelName,
                  modelId: modelName,
                  pricing: maxTokens ? `Max Tokens ${maxTokens}` : '兼容模型',
                  compatibleConfig: {
                    apiKey,
                    baseUrl,
  ...
  const typeMeta: Record<ModelType, { label: string; iconPath: string }> = {
    text: { label: '文本', iconPath: 'M4 6h16M4 12h16M4 18h7' },
    image: { label: '图像', iconPath: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
    video: { label: '视频', iconPath: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14m0-4v4m-10 4h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z' },
  };
            ...
            {activeTab === 'model-provider' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-base font-semibold text-gray-900">厂商资源池</h3>
                    <p className="mt-1 text-sm text-gray-500">参考原型图的供应商管理视图，支持连接状态与模型启停。</p>
                  </div>
                    ...
                    return (
                      <article key={provider.id} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                        <header className="flex items-center justify-between gap-3">
                          ...
                          <button className="text-xs text-gray-500 hover:text-gray-700 transition-colors">开通教程</button>
                        </header>
                                ...
                                <section className="mt-3 rounded-xl border border-gray-200 bg-gray-50 p-2.5">
                                  <div className="mb-2 flex items-center justify-between">
                                    ...
                                    {provider.allowModelManagement ? (
                                      <button className="text-xs font-semibold text-gray-600 hover:text-gray-800 transition-colors">+ 添加</button>
                                    ) : (
                                      <span className="text-[11px] text-gray-500">官方模型，不支持自定义增删改</span>
                                    )}
                                  </div>

                                  <div className="max-h-44 overflow-y-auto divide-y divide-gray-200">
                                    {currentModels.length === 0 && (
                                      <div className="py-6 text-center text-xs text-gray-400">暂无模型</div>
                                    )}
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="text-base font-semibold text-gray-900">编辑兼容模型配置</h3>
              <button
                type="button"
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700">模型名称 <span className="text-red-500">*</span></label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Max Tokens（可选）</label>
                <input
                  type="number"
                  min={1}
      ...
      {deletingModel && (
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
            <h3 className="text-base font-semibold text-gray-900">确认删除模型</h3>
