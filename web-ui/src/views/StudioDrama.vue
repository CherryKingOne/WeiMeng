...
const nextExtract = () => {
  if (extractStep.value === 2) extractStep.value = 3
}
    ...
    <teleport to="body">
      ...
      >
        ...
        >
          <!-- Modal Header -->
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-[#3A3A3C]">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">剧本提炼角色 - 第 {{ extractStep }}/3 步</h3>
            <button 
              @click="closeExtractWizard"
              class="p-2 hover:bg-gray-100 dark:hover:bg-[#3A3A3C] rounded-lg transition"
            >
          ...
          <div class="p-6">
            ...
            <div v-else class="space-y-4">
              <div class="text-sm text-gray-600 dark:text-gray-300">
                确认创建以下 {{ extractedRoles.filter(x => x.selected).length }} 个角色
              </div>
              <ul class="list-disc pl-6 space-y-2 max-h-64 overflow-y-auto">
                <li 
                  v-for="r in extractedRoles.filter(x => x.selected)" 
                  :key="r.name"
